...
// @desc Search entries by keyword, tags, or mode
// @route GET /api/search
// @access Private
const searchEntries = async (req, res, next) => {
  try {
    const { keyword, tag, mode } = req.query;
    ...
    // Personal entries are only for the logged-in user
    if (mode === 'personal') {
      query.userId = req.user._id;
    }

    const results = await Entry.find(query)
      .populate('userId', 'name profilePicture')
      .sort({ createdAt: -1 });
...
// @desc Get search suggestions from entry titles and tags
