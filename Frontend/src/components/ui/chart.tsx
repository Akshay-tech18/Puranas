...
// Type for Recharts data payload
export type RechartsDataPayload = {
  name?: string;
...
export const ChartTooltipContent = React.forwardRef<
  ...
  React.ComponentProps<"div"> & {
    ...
    labelKey?: string;
    color?: string;
    labelFormatter?: (value: any, payload?: RechartsDataPayload[]) => React.ReactNode;
...
>(
  (
    ...
    const typedPayload = payload as RechartsDataPayload[] | undefined;
    ...
    return (
      ...
      >
        {tooltipLabel}
        <div className="grid gap-1.5">
          {typedPayload.map((item: RechartsDataPayload) => {
...
// Helper to extract item config from a payload.
function getPayloadConfigFromPayload(config: ChartConfig, payload: RechartsDataPayload, key: string) {
  const payloadPayload = payload?.payload ?? {};
  const rawKey =
    (typeof payload[key as keyof RechartsDataPayload] === "string" && payload[key as keyof RechartsDataPayload]) ||
    (typeof payloadPayload[key] === "string" && payloadPayload[key]) ||
    key;

  return typeof rawKey === "string" && rawKey in config
    ? config[rawKey]
